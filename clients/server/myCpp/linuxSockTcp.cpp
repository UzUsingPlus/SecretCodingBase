...
UserBhr* linuxSockTcp::jsonToUserBhr(json js,int userSocket){
	...
	switch (stage/100){
	   case 0:{
			...
			cout <<"The print in linuxSockTcp::JsonToUserBhr" << endl ; 
			cout << "here is a HallBhr" << endl ; 
			userBhr = new HallBhr();
			userBhr->analyzeJsonToStruct(js);
		  break; 
	   }case 2:{
			cout << "try to new GameBhr "<<endl ; 
			userBhr = new GameBhr();
			cout << "try to analyze" <<endl ; 
			userBhr->analyzeJsonToStruct(js);
		  	break;
	   }
	   default: 
...
int linuxSockTcp::Send(string data, int cliSock) {
        ...
        strcpy(buf, data.c_str());
		buf[data.size()]= '\0';
     	int length = send(cliSock,(void*)buf,sizeof(buf),0) ;   
	if ( length < 0)
            return -1;
        return 1;
}
