...
SerReply* Hall::userCtrlGame(int userId,int roomId,int userSocket,int userStage,int moveDir){
    ...
    bgr.ctrlBlock(moveDir);
    gameReply->fillReply(code,userId,userSocket,UPDATE_GAME_INFO_BHR);
    bgr.buildHelper(gameReply);
    //gameReply->buildGameReply(bgr.endMark,bgr.score,bgr.ball,bgr.block);
    return gameReply ; 
}
...
vector<GameReply*> Hall::buildGRVec(){
    vector<GameReply*> GRVec ; 
    for(int i = 0 ; i < this->ballGameRoomList.size() ; i++ ){
        //whether or not need to lock up ..?
        BallGameRoom &br = this->ballGameRoomList[i];
        if( !br.returnEndMark()){
            GameReply* gr = new GameReply();
            br.buildHelper(gr);//the userid userSocket and ball block endMark score
            gr->code = BHR_SUCC  ;
            gr->userStage = UPDATE_GAME_INFO_BHR ;
            GRVec.emplace_back(gr);
        }
    }
    return GRVec ;
}
